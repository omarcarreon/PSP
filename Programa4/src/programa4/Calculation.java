...
public class Calculation {
    ...
    public static final Double dE = 0.00000001d; // Guarda tolerancia de error
    public static final Double dPI = 3.1416d; // Valor de pi
    public static Integer iNumSeg = 10; // Guarda numero de segmentos
    ...
    public static Double calculateDistT(Double dX, Integer iDof) {
        ...
        while (dDiff < dE) {
            ...
            while (iIndex <= iNumSeg) {
                ...
                dDistTAcum = dDistTAcum + (dW / 3.0) * iMultiplier * dResultFx;
                iIndex = iIndex + 1;
            }
    ...
    public static Double calculatePart1(Integer iDof) {
        Double dResult = 0.0d;
        dResult = 1 + (Math.pow((dW * iIndex), 2) / (iDof * 1.0));
        return dResult;
    }
    ...
    public static Double calculatePart2(Integer iDof) {
        Double dResult = 0.0d;
        dResult = Math.pow(dResultPart1, -((iDof + 1) / 2.0));
        return dResult;
    }
    ...
    public static Double calculatePart3(Integer iDof) {
        ...
        dResPartial1 = getGamma(((iDof + 1) / 2.0));
        dResPartial2 = Math.pow(iDof * dPI, 0.5);
        dResPartial3 = getGamma((iDof / 2.0));
    ...
    public static Double getGamma(Double dGammaParam) {
        Double dResult = dGammaParam - 1;
        Double dIterate = dGammaParam - 1;
        for (;dIterate > 0;dIterate -= 1) {
            if (dIterate == 1) {
                dResult = dResult * 1.0;
            } else if (dIterate == 0.5) {
                dResult = dResult * Math.sqrt(dPI);
            } else {
                dResult = dResult * (dIterate - 1);
            }
        }
        return dResult;
    }
